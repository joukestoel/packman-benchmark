...
cudf::AST::Package implode(cudf::Syntax::Package p) = package("<p.name>", getVersion(props), getDepends(props), getConflicts(props), getProvides(props), getInstalled(props), getKeepValue(props))
   when list[PackageProperty] props := [implode(prop) | prop <- p.stanza.properties];
   
cudf::AST::Request implode(cudf::Syntax::Request r) = request("<r.name>", [implode(prop) | prop <- r.stanza.properties]);
...
default KeepValue getKeepValue(list[PackageProperty] properties) = none(); 
